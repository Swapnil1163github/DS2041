//SINGLY LINKED LIST WITH TWO EXTERNAL REFERENCE POINTER
#include <stdio.h>
#include <stdlib.h>
...
int main() {
  ...
  do {
    ...
    case 4: // DELETE AT START
      if (start == NULL) {
        printf("LINKED LIST IS EMPTY,DELETE CAN NOT BE PERFORM\n");
    ...
    case 6: // DELETE AT GIVEN LOCATION
      ...
      scanf("%d",&loc);
      if (start == NULL) {
        printf("LINKED LIST IS EMPTY , DELETE CAN NOT BE PREFORM\n");
      } else {
        if (loc == 1) {
          p = start;
          start = start->next;
